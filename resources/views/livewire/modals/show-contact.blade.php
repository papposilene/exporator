<div id="modalShowContact" class="relative float-right mr-2">
    <div id="modalButtonShowContact">
        <a id="modalOpenShowContact" class="hover:text-red-600 dark:hover:text-red-400 cursor-pointer"
            title="@ucfirst(__('app.contact'))">
            @ucfirst(__('app.contact'))
        </a>
    </div>

    <div id="modalWindowShowContact" class="invisible fixed top-0 left-0 w-screen h-screen flex items-center justify-center bg-bluegray-900 bg-opacity-50 transform scale-0 transition-transform duration-300 z-1000">
        <!-- Modal -->
        <div class="bg-white dark:bg-gray-700 dark:text-white overflow-auto w-1/2 h-1/2 p-12">
            <!-- Close modal button-->
            <button id="modalCloseShowContact" type="button" class="focus:outline-none float-right">
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            const modalOpenShowContact = document.getElementById('modalOpenShowContact')
            const modalCloseShowContact = document.getElementById('modalCloseShowContact')
            const modalWindowShowContact = document.getElementById('modalWindowShowContact')

            modalOpenShowContact.addEventListener('click',()=>modalWindowShowContact.classList.remove('invisible'))
            modalCloseShowContact.addEventListener('click',()=>modalWindowShowContact.classList.add('invisible'))
        })
    </script>
</div>
