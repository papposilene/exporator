<div id="modalEditTag" class="relative float-right mr-2">
    <div id="modalButtonEditTag">
        <button id="modalOpenEditTag" class="focus:outline-none bg-indigo-100 text-black bg-opacity-75 px-1 rounded"
        ...
        <div class="bg-white overflow-auto w-1/2 h-1/2 p-12">
            ...
            <form method="POST" action="{{ route('admin.tag.store') }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-x-4 mt-4">
                    ...
                    <x-forms.input id="type" class="block mt-1 w-full" type="text" name="type" required value="{{ $tag->type }}" />
                    <x-forms.input id="name" class="block mt-1 w-full" type="text" name="name" required value="{{ $tag->name }}" />
                </div>
