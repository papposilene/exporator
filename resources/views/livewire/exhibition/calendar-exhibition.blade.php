@section('title', @ucfirst(__('app.calendar_of', ['what' => __('app.exhibitions')])))

<div>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-bluegray-800 dark:text-bluegray-100 leading-tight">
            <span id="top">@ucfirst(__('app.calendar_of', ['what' => __('app.exhibitions')]))</span>
        </h2>
    </x-slot>
    ...
    <div>
        <div class="max-w-7xl mx-auto py-5 px-6">
            ...
            <div class="flex flex-wrap bg-bluegray-500 mb-2 w-full">
                @for ($ii = 0; $ii < ($remaining_days + 1); $ii++)
                @php
                $day = \Carbon\Carbon::today()->add($ii, 'day');
                @endphp
                <a href="#day-{{ $ii }}" class="flex-initial text-white hover:bg-bluegray-100 hover:text-black p-3">
                    @ucfirst(__('date.' . lcfirst($day->format('l')))) {{ $day->format('d') }}
                </a>
                @endfor
            </div>
            <h3 class="bg-bluegray-500 p-3 text-2xl font-bold cursor-pointer">
            ...
            <div class="h-screen overflow-auto" x-data="{selected:null}">
                ...
                <h4 id="day-{{ $ii }}" @click="selected !== {{ $ii }} ? selected = {{ $ii }} : selected = null"
                    class="bg-bluegray-400 border-b-2 border-t-2 border-black p-3 text-2xl font-bold cursor-pointer">
                    @ucfirst(__('date.' . lcfirst($day->format('l')))) {{ $day->format('d') }}
                ...
                <div class="bg-bluegray-200 grid grid-cols-1 md:grid-cols-3 md:gap-4" x-show="selected == {{ $ii }}">
                    ...
                    <a href="{{ route('front.exhibition.show', ['place' => $exhibition_of_the_day->inPlace->slug, 'slug' => $exhibition_of_the_day->slug]) }}"
                            ...
                            class="flex flex-col bg-bluegray-300 p-2 hover:bg-green-200">
                        <div class="italic">{{ $exhibition_of_the_day->name }}</div>
